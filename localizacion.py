...
def procesar_analisis_regex():
    """Lee el cuadro de texto superior, procesa las expresiones regulares y actualiza la consola derecha."""
    # Extraer y limpiar las plantillas ingresadas por el usuario
    contenido_input = txt_plantillas.get("1.0", tk.END).strip()
    
    # Expresión regular para separar por comas e ignorar saltos de línea, espacios o comillas extras
    ...
    # Analizar el set actual de capturas en memoria
    for idx, item in enumerate(capturas):
        ...
        for plantilla in plantillas_objetivo:
            ...
            if re.search(patron, ruta_limpia):
                ...
                break  # Detener evaluación al encontrar su respectiva plantilla

    # Filtro estricto: Eliminar las llaves de plantillas que se quedaron vacías []
    reporte_limpio = {plantilla: datos for plantilla, datos in reporte.items() if len(datos) > 0}

    # Renderizar el JSON resultante en el cuadro derecho
    txt_json_filtrado.delete("1.0", tk.END)
    txt_json_filtrado.insert("1.0", json.dumps(reporte_limpio, indent=4, ensure_ascii=False))
...
def copiar_filtrado():
    ...
    messagebox.showinfo("Copiado", "¡JSON Filtrado Comparativo copiado!")
...
def guardar_ambos_archivos():
    ...
    messagebox.showinfo("Guardado", "¡Ambos archivos JSON ('capturas.json' y 'reporte_plantillas.json') exportados con éxito!")


# --- DISEÑO DE LA INTERFAZ DE USUARIO (TKINTER) ---
root = tk.Tk()
root.title("Capturador de Servicios API & Comparador Regex Avanzado")
root.geometry("1200x850")  # Ventana más ancha para las dos columnas
...
# Panel Superior 2: Entrada masiva de Plantillas Objetivo (Input de texto grande)
frame_input_plantillas = ttk.LabelFrame(root, text=" Lista de Plantillas URLs Objetivo (Separadas por comas o líneas, admite comillas) ", padding=10)
...
# Inserción de tus valores predeterminados para facilitar las pruebas iniciales
...
# --- SECCIÓN CENTRAL - LAS DOS CONSOLAS EN PARALELO (LADO A LADO) ---
...
if __name__ == "__main__":
    # Recordar ejecutar usando: SYSTEM_VERSION_COMPAT=1 python3 script.py si persiste el aviso de versión en macOS
    root.mainloop()