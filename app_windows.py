...
# 🌟 CONFIGURACIÓN DE RUTAS DINÁMICAS PARA WINDOWS (PyInstaller)
# Evita que el ejecutable pierda el rumbo al abrir subprocesos en las carpetas temporales de Windows
...
def plantilla_a_regex(plantilla):
    """Convierte formatos como {id} en patrones regex de forma segura."""
...
def procesar_analisis_regex():
    ...
    # Estructura temporal para acumular開coincidencias
    ...
    json_final_combinado = {
        "resumen_conteos": resumen_conteos,
        "detalle_peticiones": reporte_limpio
    }
...
def bucle_playwright():
    ...
    try:
        ...
        # En Windows forzamos el uso de Google Chrome nativo de la máquina
...
def guardar_ambos_archivos():
    ...
    messagebox.showinfo("Guardado", "¡Ambos archivos exportados con éxito!")
...
frame_input_plantillas = ttk.LabelFrame(root, text=" Lista de Plantillas URLs Objetivo ", padding=10)
...
btn_guardar = ttk.Button(frame_acciones, text="💾 Guardar ambos archivos JSON", command=guardar_ambos_archivos)
